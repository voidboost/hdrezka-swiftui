    ...
    private func setupPlayer(seek: CMTime? = nil, isPlaying playing: Bool = true, isMuted muted: Bool = false, subtitles: String? = nil, completion: (() -> Void)? = nil, volume vol: Float = 1.0) {
        ...
        if let player, let currentItem = player.currentItem {
            ...
            if let thumbnails = movie.thumbnails {
                movieDetails
                    .getMovieThumbnails(path: thumbnails)
...
//                .store(in: &subscriptions)
    ...
    private func prevTrack(_ seasons: [MovieSeason], _ season: MovieSeason, _ episode: MovieEpisode) {
        if let prevEpisode = season.episodes.element(before: episode) {
            ...
            resetPlayer {
                movieDetails
                    .getMovieVideo(voiceActing: voiceActing, season: season, episode: prevEpisode, favs: favs)
                    .receive(on: DispatchQueue.main)
                    .sink { completion in
                        ...
                        if movie.needPremium {
                            ...
                            if isLoggedIn {
                                account
                                    .saveWatchingState(voiceActing: voiceActing, season: season, episode: prevEpisode, position: 0, total: 0)
            ...
            resetPlayer {
                movieDetails
                    .getMovieVideo(voiceActing: voiceActing, season: prevSeason, episode: prevEpisode, favs: favs)
                    .receive(on: DispatchQueue.main)
                    .sink { completion in
                        guard case let .failure(error) = completion else { return }
                        ...
                        if movie.needPremium {
                            ...
                            if isLoggedIn {
                                account
                                    .saveWatchingState(voiceActing: voiceActing, season: prevSeason, episode: prevEpisode, position: 0, total: 0)
                                    .sink { _ in } receiveValue: { _ in }
                                    .store(in: &subscriptions)
                            }
    ...
    private func nextTrack(_ seasons: [MovieSeason], _ season: MovieSeason, _ episode: MovieEpisode) {
        if let nextEpisode = season.episodes.element(after: episode) {
            ...
            resetPlayer {
                movieDetails
                    .getMovieVideo(voiceActing: voiceActing, season: season, episode: nextEpisode, favs: favs)
                    .receive(on: DispatchQueue.main)
                    .sink { completion in
                        guard case let .failure(error) = completion else { return }
                        ...
                        if movie.needPremium {
                            ...
                            if isLoggedIn {
                                account
                                    .saveWatchingState(voiceActing: voiceActing, season: season, episode: nextEpisode, position: 0, total: 0)
                                    .sink { _ in } receiveValue: { _ in }
                                    .store(in: &subscriptions)
                            }
            ...
            resetPlayer {
                movieDetails
                    .getMovieVideo(voiceActing: voiceActing, season: nextSeason, episode: nextEpisode, favs: favs)
                    .receive(on: DispatchQueue.main)
                    .sink { completion in
                        guard case let .failure(error) = completion else { return }
                        ...
                        if movie.needPremium {
                            ...
                            if isLoggedIn {
                                account.saveWatchingState(voiceActing: voiceActing, season: nextSeason, episode: nextEpisode, position: 0, total: 0)
                                    .sink { _ in } receiveValue: { _ in }
                                    .store(in: &subscriptions)
                            }
