    ...
    var body: some View {
            ...
            VStack(alignment: .center, spacing: 10) {
                Button {
                    if let details, let selectedActing, let selectedQuality, let movie {
                        if isLoggedIn {
                            account
                                .saveWatchingState(voiceActing: selectedActing, season: selectedSeason, episode: selectedEpisode, position: 0, total: 0)
            ...
            if let details, let selectedActing {
                if details.series != nil {
                    if let movieId = details.movieId.id {
                        movieDetails
                            .getSeriesSeasons(movieId: movieId, voiceActing: selectedActing, favs: details.favs)
                            .receive(on: DispatchQueue.main)
                            .sink { completion in
                                guard case let .failure(error) = completion else { return }
                    ...
                    movieDetails
                        .getMovieVideo(voiceActing: selectedActing, season: nil, episode: nil, favs: details.favs)
                        .receive(on: DispatchQueue.main)
                        .sink { completion in
                            guard case let .failure(error) = completion else { return }
        ...
        .onChange(of: selectedEpisode) {
            ...
            if let details, let selectedSeason, let selectedEpisode, let selectedActing {
                movieDetails
                    .getMovieVideo(voiceActing: selectedActing, season: selectedSeason, episode: selectedEpisode, favs: details.favs)
                    .receive(on: DispatchQueue.main)
                    .sink { completion in
                        guard case let .failure(error) = completion else { return }
